...
def checking_list_found(context):
    ...
    context.all_link = wait.until(
        ec.presence_of_all_elements_located((By.XPATH, "//*[@class='Nv2PK THOPZb CpccDe ']/child::a")))
    assert context.all_link != [], 'Unable To Find Required List'


@then('He Save Top "{total_count}" All The Details')
def saving_details(context, total_count):
    ...
    make_csv(set(tot))
    with open('Google_Map5.csv', 'a') as csvfile:
